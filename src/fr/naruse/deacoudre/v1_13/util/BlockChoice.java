...
public class BlockChoice {
    ...
    public BlockChoice(){
        for(int i = 1; i <= 10; i++){
            ...
            inventoryIntegerHashMap.put(inv, i-1);
        }
        ItemStack item;
    ...
    public boolean contains(Inventory inv){
        return invs.contains(inv);
    }

    public int getInventoryPlace(Inventory inv){
        if(inventoryIntegerHashMap.containsKey(inv)){
            return inventoryIntegerHashMap.get(inv);
        }
        return 0;
    }
}
